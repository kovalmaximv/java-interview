...
- [Блокировки и deadlock в БД](#блокировки-и-deadlock-в-бд)
- [Репликации](#репликации)
- [Партиционирование](#партиционирование)
- [OLAP vs OLTP](#olap-vs-oltp)
- [Способы оптимизации SQL запросов](#способы-оптимизации-sql-запросов)
- [Порядок выполнения SQL команд](#порядок-выполнения-sql-команд)
- [Работа SQL оптимизатора](#работа-sql-оптимизатора)
...
Чем выше уровень изолированности транзакций, тем выше шанс появления серьезных взаимных блокировок. Если приложение 
большое, sql запросы исчисляются сотнями, то возможно имеет смысл не тратить много времени на предотвращение 
маловероятных и некритичных дедлоков. В таком случае можно на уровне кода поставить try-catch и перезапустить запрос.

Для более сложных кейсов следующие советы:

#### Единый порядок
Так же необходимо захватывать монитор (мьютекс/lock) на объекты в неком едином порядке.

#### UPDLOCK/SELECT FOR UPDATE
Если две параллельных транзакции начнут читать ресурс и наложат на него Shared блокировку, а потом обе транзакции 
захотят что-то в этом ресурсе изменить, то они оба будут ждать когда спадет соседняя Shared блокировка. В таком случае
первый запрос на чтение в транзакции должен быть с UPDATE локом. В MS SQL это конструкция `SELECT ... WITH (UPDLOCK) ...`,
в PostgreSQL это `SELEC .. FOR UPDATE`. Накладывая update блокировку мы заставляем все транзакции, обращающиеся к 
одному ресурсу, выполняться по очереди.

#### Выборки по неиндексируемым полям
Если serializable транзакция запрашивает выборку по неиндексируемому полю, то shared блокировка накладывается на всю 
таблицу. Если же добавить индекс на первое поле из запроса, то блокироваться будет ключ в этом индексе. Если ключа нет,
то на соседние ключи в индексе.
Последнее может стать проблемой, если таблица сильно разряжена (например ближайшие ключи в индексе 1 и 100500), в таком
случае лучше использовать UPDLOCK/SELECT FOR UPDATE.

## Репликации
Репликация - хранение копии одних и тех же данных на нескольких машинах. Существует несколько причин для репликации:
1) Надежность при хранении данных
2) Ради хранения данных географически ближе к пользователю
3) Чтобы система могла работать при отказе некоторых систем
4) Для увеличения количества машин, способных принимать запросы на чтение

Основная сложность репликации в том, чтобы поддерживать репликации актуальными. Существуют три алгоритма
...
заменить лидера. Недостаток - замедление быстродействия.
...
шанс аварии. При асинхронной записи данные, прочитанные с реплики, могут быть устаревшими.

Так же существует подход с несколькими ведущими узлами. Такая модель называется multi-leader replication или реплика
master-master. Можно установить ведущий узел в каждом из ЦОДов. Внутри каждого ЦОДа есть ведущая и ведомые реплики. 
Ведущие двух ЦОДов обмениваются информацией друг с другом. Вот плюсы такой схемы:
...
Горизонтальное партицирование (когда база режется построчно по **разным** машинам) == Шардирование. Партиционирование 
еще может быть вертикальным, когда таблицу делят по столбцам, такое используется редко, как правило перед разделением
таблицы на несколько, для последующего полного разделения.

Шардирование позволяет улучшить производительность БД, поскольку:
1) Данные равномерно разбиваются на шарды. Запросы на шардах выполняются параллельно. Результаты запросов со всех 
шардов собирает воедино координатор.
2) Если в запросе фигурирует ключ шардирование и необходимые данные можно найти на одном шарде, то запрос выполняется
только на одном шарде.

В PostgreSQL нет стандартных инструментов для шардирования, но есть инструменты по типу Citus или pg_shard.
...
2) По хешу - в зависимости от выбранного ключа, может быть норм, если ключ не повторяется и хорошо разбросан 
