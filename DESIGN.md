...
3) Пользователи используют много совместных одинаковые данные (например каталог продуктов, справочник, итд)
...
4) Клиент может выполнять необходимую работу, у него осталось доступное время для удержания замка - время которое было
потрачено на захват замка
...
Каждый из методов можно реализовать с помощью hashmap и очереди с приоритетом. В мапе будут хранится сами элементы кеша, 
а в очереди с приоритетами будут хранится ключи в порядке их использования. В самом начале (а значит самый маленький),
ключ который использовался очень давно, в самом конце ключ который использовался совсем недавно.