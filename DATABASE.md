...
2) Неключевые атрибуты зависят от ключа целиком.
...
Создание индекса - долго и **блокирующая** операция.  
Есть неблокирующий вариант, но результат не гарантируется.
...
По кластеризации:
+ Кластерный - задает порядок (напр Primary Key)
+ Некластерный - содержит ток указатели, не задает порядок (напр Unique)
...
2) read committed - чтение всех зафиксированных изменений параллельных транзакций. Потерянные изменения и грязное 
чтение не допускается, возможны неповторяемое чтение и фантомы.
3) repeatable read — чтение изменений только своей транзакции. Потерянные изменения, грязное и неповторяемое чтение 
невозможны, возможны фантомы (потому что lock не работает на диапазон данных).
...
Горизонтальное партицирование (когда база режется построчно) == Шардирование.
...
3) P (partition tolerance) — устойчивость к распределению. Даже если между узлами нет связи, они продолжают работать 
